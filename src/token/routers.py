...
@router.post("/refresh")
async def refresh(token_service: TokenService = Depends(get_token_service)) -> JSONResponse:
    """
    ...
    refresh_token: str = token_service.create_token(user_id=user_id, token_type=TokenType.REFRESH)
    access_token: str = token_service.create_token(user_id=user_id, token_type=TokenType.ACCESS)
    return JSONResponse(
...
async def validate_token(
    ...
    is_valid: bool = token_service.validate_token(token=token_dict.get("token"))
    if is_valid:
